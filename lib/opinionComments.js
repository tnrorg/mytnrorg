 ...
 *
 * Editing is for the typo you spot immediately, not for rewriting what you
 * said after people have replied to it. Fifteen minutes covers the first, and
 * the "edited" mark covers the rest — a comment that silently changed after
 * three people answered it rewrites the conversation around them. */
export const EDIT_WINDOW_MINUTES = 15;

export function withinEditWindow(createdAt) {
  if (!createdAt) return false;
  return (Date.now() - new Date(createdAt).getTime()) < EDIT_WINDOW_MINUTES * 60_000;
}

/** Only the writer, and only while the window is open. */
export const canEdit = ({ comment, viewerMemberId }) =>
  !!viewerMemberId && comment?.member_id === viewerMemberId && withinEditWindow(comment?.created_at);
