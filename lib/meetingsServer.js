...
import {
  effectiveStatus, endsAt, typeLabel, fmtMeetingTime, AUDIENCE_KEYS,
} from '@/lib/meetings';
...
export const MEETING_NOTICES = {
  created:     (m) => [`${typeLabel(m.meeting_type)} scheduled`,
    `${m.title} — ${fmtMeetingTime(m.scheduled_at)}.`],
  reminder:    (m) => ['Your meeting starts soon',
    `${m.title} begins at ${fmtMeetingTime(m.scheduled_at)}.`],
  started:     (m) => ['A meeting is now live',
    `${m.title} has started. Join from My Meetings.`],
  cancelled:   (m) => ['Meeting cancelled',
    `${m.title}, scheduled for ${fmtMeetingTime(m.scheduled_at)}, has been cancelled.`],
  rescheduled: (m) => ['Meeting rescheduled',
    `${m.title} has moved to ${fmtMeetingTime(m.scheduled_at)}.`],
...
/* ── Reminders ────────────────────────────────────────────────────────────
 *
 * "Your meeting starts in an hour."
 *
 * NO SCHEDULER EXISTS IN THIS PROJECT — no cron, no queue, no background
 * worker. So this runs opportunistically, on the same list reads that already
 * sweep the lifecycle. In practice a member portal or admin panel is opened
 * many times an hour, which is enough for a reminder measured in hours.
 *
 * The honest limitation: if literally nobody opens the site between the
 * meeting being scheduled and it starting, the reminder does not go. That is
 * a real gap, and the fix is a scheduled job — recorded here rather than
 * papered over, because a reminder people believe in but that silently does
 * not fire is worse than none.
 *
 * `reminded_at` on the meeting is what makes it send ONCE. Without it every
 * page load in the hour before a meeting would notify 293 people again.
 */
export const REMIND_BEFORE_MINUTES = 60;

export async function sendMeetingReminders() {
  const sb = supabaseAdmin();
  const now = Date.now();
  const window_ = new Date(now + REMIND_BEFORE_MINUTES * 60000).toISOString();

  const { data: due, error } = await sb.from('meetings')
    .select('id, title, meeting_type, scheduled_at, reminded_at')
    .eq('status', 'scheduled')
    .is('reminded_at', null)
    .gt('scheduled_at', new Date(now).toISOString())
    .lte('scheduled_at', window_);

  // The column arrives in a later migration; until it is run this does nothing
  // rather than breaking the list read it is riding on.
  if (error || !due?.length) return { sent: 0 };

  let sent = 0;
  for (const m of due) {
    /* Claim it BEFORE notifying.
     *
     * Two admins refreshing at the same moment would otherwise both find it
     * unreminded and both send. The conditional update is the lock: only one
     * request can turn a null into a timestamp, and only that one goes on to
     * notify. */
    const { data: claimed } = await sb.from('meetings')
      .update({ reminded_at: new Date().toISOString() })
      .eq('id', m.id).is('reminded_at', null)
      .select('id').maybeSingle();
    if (!claimed) continue;

    const r = await notifyMeeting(m, 'reminder');
    sent += r.sent || 0;
  }
  return { sent, meetings: due.length };
}
