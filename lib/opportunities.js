...
export const FELLOWSHIP_QUESTIONS = [
  /* Academic standing first.
   *
   * These are the two fields the committee will actually sort and filter on,
   * and they belong with the applicant's studies rather than after a run of
   * logistics questions about laptops and internet. */
  {
    key: 'semester',
    label: 'Which semester or year of study are you currently in?',
    options: [
      '1st Semester', '2nd Semester', '3rd Semester', '4th Semester',
      '5th Semester', '6th Semester', '7th Semester', '8th Semester',
      'Graduated', 'Not currently studying',
    ],
  },
  {
    key: 'cgpa',
    label: 'What is your current CGPA or percentage?',
    /* Free text, not a list.
     *
     * Universities here use a 4.0 scale and a percentage more or less
     * interchangeably, and forcing one would either exclude half the
     * applicants or make them convert — which is where wrong numbers come
     * from. Both are accepted and the validator recognises either. */
    input: 'text',
    placeholder: 'e.g. 3.45 or 82%',
    hint: 'Enter it as your transcript shows — a CGPA out of 4.00, or a percentage.',
    optionalWhen: { key: 'semester', is: ['Not currently studying'] },
  },
  {
    key: 'internet',
  ...
  },
  {
    ...
    /* Leadership only.
     *
     * This is a leadership fellowship, so the other options were offering a
     * choice the programme does not actually have. An applicant who picks
     * "Entrepreneurship" has been invited to expect something the fellowship
     * will not give them. */
    options: ['Leadership'],
  },
];
 ...
 */
/* A CGPA on either scale.
 *
 * Accepts 0–4 (grade point) or 0–100 (percentage), with or without a % sign.
 * Deliberately permissive about FORM and strict about RANGE: rejecting "3.45"
 * because someone typed "3.45/4" helps nobody, but accepting "345" would put a
 * meaningless number in front of the committee. */
export function parseCgpa(raw) {
  const s = String(raw ?? '').trim();
  if (!s) return { ok: false, empty: true };
  const m = s.match(/^(\d{1,3})(?:[.,](\d{1,2}))?\s*%?(?:\s*\/\s*\d+(?:\.\d+)?)?$/);
  if (!m) return { ok: false, reason: 'Enter a number, for example 3.45 or 82%.' };
  const n = Number(`${m[1]}.${m[2] || 0}`);
  if (Number.isNaN(n)) return { ok: false, reason: 'Enter a number, for example 3.45 or 82%.' };
  if (n <= 4.0) return { ok: true, value: n, scale: 'gpa' };
  if (n <= 100) return { ok: true, value: n, scale: 'percent' };
  return { ok: false, reason: 'That looks too high — use a CGPA out of 4.00 or a percentage.' };
}

/** Is this question required, given the other answers? */
function isRequired(q, answers) {
  if (!q.optionalWhen) return true;
  const other = String(answers[q.optionalWhen.key] ?? '').trim();
  return !q.optionalWhen.is.includes(other);
}
...
export function validateAnswers(answers = {}, declarationAccepted = false) {
  ...
  for (const q of FELLOWSHIP_QUESTIONS) {
    ...
    // Free-text questions (CGPA). Skipped entirely when not required — a
    // graduate has no current semester GPA to give.
    if (q.input) {
      if (!v) { if (required) e[q.key] = 'This is required.'; continue; }
      if (q.key === 'cgpa') {
        const r = parseCgpa(v);
        if (!r.ok) e[q.key] = r.reason || 'Please check this value.';
      } else if (v.length > 120) {
        e[q.key] = 'Please keep this under 120 characters.';
      }
      continue;
    }

    // Option questions.
    if (!v) { if (required) e[q.key] = 'Please choose an option.'; continue; }
