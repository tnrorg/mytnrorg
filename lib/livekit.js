...
/* KEEP THE RECORDING BUCKET PRIVATE.
 *
 * These files are interviews with named candidates and Advisory Council
 * sessions. A public bucket means anyone holding the URL — a forwarded email,
 * a browser history, a shared screen — has the recording for ever, with no way
 * to withdraw it.
 *
 * Private has one consequence worth knowing: the URL LiveKit reports when the
 * upload finishes is a plain object URL, and against a private bucket it
 * answers 403. Playback and AI transcription therefore go through
 * signedRecordingUrl() in lib/supabaseServer.js, which signs at the moment of
 * use so no long-lived link to a recording is ever stored.
 */
