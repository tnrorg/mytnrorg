 ...
 * anyway.
 *
 * WHERE THE FILE LANDS is a separate decision from whether recording works at
 * all. With no storage configured, LiveKit Cloud keeps the file in its own
 * bucket and hands back a URL; that is fine for a committee that records
 * occasionally. An organisation that wants its recordings under its own
 * control sets the four S3 variables below and they go straight there.
 *
 * Either way TNR stores only the METADATA — who started it, how long, where
 * the file is. Recordings of Advisory Council sessions are not something to
 * copy around more than necessary.
 */
function egress() {
  const { url, key, secret, configured } = livekitConfig();
  if (!configured) throw new Error('LIVEKIT_NOT_CONFIGURED');
...
/** Optional S3 destination. Absent means LiveKit Cloud's own storage. */
function fileOutput(filepath) {
  const out = new EncodedFileOutput({ fileType: EncodedFileType.MP4, filepath });
  ...
  if (bucket) {
    out.output = {
      ...
      value: {
        ...
        bucket,
        ...(process.env.LIVEKIT_S3_ENDPOINT ? { endpoint: process.env.LIVEKIT_S3_ENDPOINT } : {}),
      },
    };
  }
  return out;
...
export async function startAudioRecording(room, meetingId) {
  ...
  const out = new EncodedFileOutput({
    fileType: EncodedFileType.OGG,          // Opus: small, and Whisper reads it
    filepath: `tnr/${meetingId}/${stamp}-audio.ogg`,
  });
