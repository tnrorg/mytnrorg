...
/* ── datetime-local ↔ UTC ──────────────────────────────────────────────────
 *
 * An <input type="datetime-local"> reads and writes a NAIVE string —
 * "2026-08-17T20:00", with no timezone attached. Postgres stores timestamptz,
 * and interprets a naive value as UTC.
 *
 * So an editor in Pakistan choosing 20:00 had it saved as 20:00 UTC, which is
 * 01:00 the next morning for them: five hours in the future. The post was
 * published, correct in every other way, and simply would not appear — the
 * same symptom as a broken query, which is what made it worth naming here.
 *
 * These two convert explicitly. Everything stored is UTC; everything shown in
 * the picker is the editor's own clock.
 */

/** UTC ISO → the 'YYYY-MM-DDTHH:mm' the picker expects, in local time. */
...
/** Picker value (local) → UTC ISO for storage. */
export function fromLocalInput(v) {
  if (!v) return null;
  const d = new Date(v);            // parsed in the browser's own timezone
