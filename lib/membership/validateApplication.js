...
export function validateApplication(f = {}) {
  ...
  // The photo appears on the membership card, so it is required rather than
  // optional. Checked by shape, not just presence — an empty string or a
  // non-image data URL would otherwise pass.
  const photo = str('photo_data');
  if (!photo) e.photo_data = 'A profile photo is required.';
