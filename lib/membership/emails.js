...
/**
 * Base address for links inside emails.
 *
 * The fallback used to be an old preview deployment (tnr-nine.vercel.app).
 * When NEXT_PUBLIC_SITE_URL was not set, every password-reset and invitation
 * link pointed at a URL Vercel had long since deleted — the member received
 * the email, clicked, and got DEPLOYMENT_NOT_FOUND. A stale fallback is worse
 * than no fallback, because nothing looks broken until someone tries to use it.
 *
 * Order: an explicit setting, then the deployment's own hostname (correct on
 * any preview or production build), then the live domain.
 */
const siteUrl = () => {
