...
import { typeLabel, fmtMeetingTime, endsAt } from '@/lib/meetings';
...
export async function sendMeetingEmail({ kind, meeting, member, host }) {
  ...
  /* Pinned to the organisation's timezone and labelled.
   *
   * This email is composed on a server running in UTC — the ambient formatter
   * rendered a 11:00 PM Pakistan meeting as the UTC clock face, which is a
   * different hour on a different day. */
  const when = fmtMeetingTime(meeting.scheduled_at);
  const body = [
    ...
    /* An email is written once for everyone, so its text cannot be in each
     * member's own timezone. The attachment can — every calendar converts a
     * UTC entry automatically — so members abroad are pointed at it rather
     * than left doing the arithmetic. */
    kind === 'cancelled' ? '' :
      '\nIf you are outside Pakistan, add the attached calendar entry and it will '
      + 'show in your own local time.',
 ...
 *
 * Each is an SMTP round trip — typically one to two seconds against Gmail,
 * more when it is slow. Twenty-five did not fit in a serverless function's
 * time limit: the request was killed part-way, so some members were emailed,
 * the flag for the rest was never written, and the caller was left waiting on
 * a response that never came. Ten fits with room to spare, and the caller
 * loops. Slower to watch, but it finishes, and it reports who was reached. */
export const EMAIL_CHUNK = 10;
