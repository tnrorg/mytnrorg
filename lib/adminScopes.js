...
export const SCOPES = [
  ...
  /* Its own area, and a narrow one.
   *
   * This screen answers "how active is each member" for all 293 of them by
   * name. That is a judgement about people, it is the kind of thing that gets
   * quoted in an election, and it should be reachable by the office bearers
   * who need it rather than by everyone who happens to hold an admin login. */
  { key: 'analytics', label: 'Progress Analytics', icon: '📈',
    hint: 'Member and leadership participation, attendance over the year, activity log' },
];
...
export const ROUTE_SCOPES = {
  ...
  dashboard: ANY,
  /* Changing YOUR OWN password. Any signed-in admin, deliberately — the whole
   * point is that securing your own account does not require asking someone
   * with more permissions than you. The route verifies the current password
   * and only ever touches the caller's own row. */
  password: ANY,
  ...
  // ── Progress Analytics ──
  // Contribution records, the manual activity log, and the year export.
  analytics: ['analytics'],
