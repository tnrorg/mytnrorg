 ...
 */
export function attendanceStatusFor({ totalSeconds, runSeconds, firstJoinedAt, startedAt }) {
  ...
  // Full attendance, but did they arrive on time? Measured from when the
  // meeting actually started, not from when it was scheduled — a host who
  // opened twenty minutes late must not make every attendee "late".
  const late = startedAt && firstJoinedAt
    && (new Date(firstJoinedAt) - new Date(startedAt)) / 60000 > ATTENDANCE_RULES.lateAfterMinutes;
