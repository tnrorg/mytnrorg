 ...
 *
 * COUNT WHAT YOU MEAN TO LIMIT.
 * `count: false` checks the current standing without adding to it. That lets a
 * caller charge the limit only for requests that actually did the expensive
 * thing, instead of charging for every attempt — including the ones rejected
 * for a mistyped field, which cost nothing and are how honest people fill in
 * forms.
 *
 * @returns {{ blocked: boolean, retryAfter?: number }}
 */
export async function throttle(scope, ip, {
  max = 5, windowMinutes = 15, lockMinutes = 15, count = true,
} = {}) {
  ...
  try {
    ...
    const prior = windowOpen ? (row?.fails || 0) : 0;

    // Check only: report whether the NEXT one would be refused, and write
    // nothing. Merely asking must not use up the allowance.
    if (!count) return prior >= max
      ? { blocked: true, retryAfter: lockMinutes * 60 }
      : { blocked: false };
