...
// ── The four things being counted ──────────────────────────────────────────
/* Used to render the same four groups, in the same order, with the same
 * wording, on the admin table and on the member's own page. */
...
export function engagementBand(rec) {
  const total = (rec?.meetings?.attended || 0)
